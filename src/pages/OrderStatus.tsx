  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f7f8f4] text-slate-900">
      <header className="sticky top-0 z-40 shrink-0 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="container mx-auto flex h-12 items-center justify-between gap-3 px-4 sm:px-6">
          <button type="button" onClick={scrollHome} className="flex shrink-0 items-center" aria-label="Urban Delights home">
            <img src={settings.logoUrl || "/logo.png"} alt={settings.storeName || "Urban Delights"} className="h-9 w-auto max-w-[180px] object-contain" />
          </button>

          <nav className="hidden items-center gap-5 sm:flex" aria-label="Main navigation">
            <button type="button" onClick={() => scrollToSection("products")} className="text-xs font-semibold text-stone-600 transition hover:text-orange-700">Shop</button>
            {settings.showCategories && settings.categories.length > 0 && <button type="button" onClick={() => scrollToSection("categories")} className="text-xs font-semibold text-stone-600 transition hover:text-orange-700">Categories</button>}
            {settings.showWhyUs && <button type="button" onClick={() => scrollToSection("why-us")} className="text-xs font-semibold text-stone-600 transition hover:text-orange-700">Why Us</button>}
            <button type="button" onClick={() => navigate("/order-status")} className="text-xs font-bold text-orange-700 transition hover:text-orange-800">Track Order</button>
          </nav>

          <div className="flex shrink-0 items-center gap-1.5">
            {settings.showAdminLogin && <Button variant="outline" size="icon" onClick={() => navigate("/login")} className="h-8 w-8 rounded-full" aria-label="Admin Login" title="Admin Login"><span className="sr-only">Admin Login</span><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg></Button>}
            <Button variant="outline" onClick={scrollHome} className="h-8 rounded-lg px-3 text-xs"><ArrowLeft className="mr-1.5 h-3.5 w-3.5" /> Store</Button>
          </div>
        </div>
      </header>
      ...
      <main className="min-h-0 flex-1">
        <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-6">
          ...
          <Card className="mx-auto mt-4 max-w-xl rounded-2xl border-stone-200 bg-white shadow-sm">
            <CardHeader className="px-5 pb-1 pt-4 sm:px-6"><CardTitle className="text-base">Find your order</CardTitle></CardHeader>
