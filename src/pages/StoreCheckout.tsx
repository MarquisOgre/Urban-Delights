...
const StoreCheckout = () => {
  ...
  const valid = form.name.trim() && form.phone.trim() && form.address.trim() && hydratedItems.length > 0 && (paymentMethod !== "upi" || transactionId.trim());
  ...
  const submitOrder = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("place_storefront_order" as never, {
        ...
        p_payment_status: paymentMethod === "upi" ? "pending" : "pending",
