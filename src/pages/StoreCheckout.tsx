...
<main className="flex min-h-0 flex-1 items-center"><div className="mx-auto w-full max-w-3xl px-4 py-12"><Card className="w-full rounded-2xl border-stone-200 p-8 text-center shadow-sm"><CheckCircle2 className="mx-auto h-16 w-16 text-emerald-600" /><h1 className="mt-5 font-serif text-3xl font-bold text-[#2f6b45]">Order Placed Successfully</h1><p className="mt-2 text-stone-500">Your order has been received successfully.</p><div className="mt-5 rounded-xl bg-stone-100 p-4 text-sm">Order ID: <span className="font-mono font-bold">{success.toUpperCase()}</span><div className="mt-1">Total: <span className="font-bold">₹{total.toFixed(0)}</span></div></div><div className="mt-6 flex flex-wrap justify-center gap-3"><Button onClick={goHome}><Home className="h-4 w-4" />Back to Home</Button><Button variant="outline" onClick={() => navigate("/?preview=1#products")}><ShoppingBag className="h-4 w-4" />Continue Shopping</Button></div></Card></div></main>
<Footer />
</div>
);
