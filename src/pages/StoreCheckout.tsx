...
<<<<<<< Updated upstream
  // Build the UPI payment URI from the CURRENT order total. The uploaded QR is intentionally
  // not used for payment because a static QR can contain an old amount or malformed data.
...
=======
  // Build a UPI payment intent dynamically so the QR code always requests
  // the current checkout total.
  const upiPaymentUri = useMemo(() => {
    const upiId = String(settings.upiId || "").trim();
    const payeeName = String(
      settings.upiPayeeName || settings.storeName || "Urban Delights",
    ).trim();
...
>>>>>>> Stashed changes
  ...
  // QuickChart only renders the QR image; the encoded payload is the standard UPI deep link above.
...
<<<<<<< Updated upstream
    <div className="min-h-screen bg-[#f7f8f4] text-slate-900">
      <header className="mx-auto flex max-w-[1680px] items-center justify-between px-5 py-3 sm:px-8">
=======
    <div className="flex min-h-screen flex-col bg-[#f7f8f4] text-slate-900 lg:h-screen lg:overflow-hidden">
      <header className="flex h-[76px] shrink-0 items-center justify-between border-b border-stone-200 bg-white/90 px-5 backdrop-blur sm:px-8">
...
<<<<<<< Updated upstream
      <main className="mx-auto max-w-[1680px] px-5 pb-12 pt-5 sm:px-8">
        <form onSubmit={submitOrder} className="grid items-stretch gap-7 lg:grid-cols-3">
...
=======
      <main className="min-h-0 flex-1 px-4 py-3 sm:px-6 lg:overflow-hidden lg:px-8">
        <form
          onSubmit={submitOrder}
          className="mx-auto grid h-auto max-w-[1680px] gap-4 lg:h-full lg:min-h-0 lg:grid-cols-3"
        >
          <Card className="flex min-h-0 flex-col overflow-hidden rounded-2xl border-stone-200 bg-white shadow-sm lg:h-full">
            ...
            <CardContent className="min-h-0 flex-1 overflow-hidden px-6 pb-4">
          ...
          <Card className="flex min-h-0 flex-col rounded-2xl border-stone-200 bg-white shadow-sm lg:h-full">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="relative mt-1.5">
                    <Input
                      ...
                      className="h-10 rounded-xl border-stone-200 bg-[#fbfcf8] pr-11"
                      required
                    />

                    <UserRound className="absolute right-3 top-3.5 h-5 w-5 text-stone-400" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="mt-1.5 h-10 rounded-xl border-stone-200 bg-[#fbfcf8]"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <Phone className="absolute right-3 top-3.5 h-5 w-5 text-stone-400" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    <Textarea
                      ...
                      className="min-h-[76px] rounded-xl border-stone-200 bg-[#fbfcf8] pr-11"
                      required
                    />

                    <MapPin className="absolute right-3 top-3.5 h-5 w-5 text-stone-400" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="min-h-[58px] rounded-xl border-stone-200 bg-[#fbfcf8]"
                  />
                </div>
              </div>
            </CardContent>
          ...
          <Card className="flex min-h-0 flex-col rounded-2xl border-stone-200 bg-white shadow-sm lg:h-full">
            ...
            <CardContent className="min-h-0 flex-1 overflow-hidden px-6 pb-4">
              <div className="min-h-0 flex-1 overflow-hidden">
                <div className="rounded-xl border border-[#2f6b45] bg-[#f4faf6] p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-stone-500">
                        Pay securely via UPI and enter the transaction ID below.
                      </div>
                    </div>

                    <span className="rounded-full bg-[#2f6b45] px-3 py-1 text-xs font-bold text-white">
                      UPI
                    </span>
                  </div>

                  <div className="mt-2 rounded-xl border border-stone-200 bg-white p-2.5 text-center shadow-sm">
                    ...
                    {qrCodeUrl ? (
                      <img
                        src={qrCodeUrl}
                        alt={`UPI payment QR code for ₹${total.toFixed(2)}`}
                        className="mx-auto mt-1.5 h-36 w-36 rounded-lg object-contain sm:h-40 sm:w-40"
                        loading="eager"
                      />
                    ) : (
                      <div className="mx-auto mt-1.5 flex h-36 w-36 items-center justify-center rounded-lg border border-dashed border-stone-300 bg-stone-50 px-4 text-center text-[11px] leading-tight text-stone-500 sm:h-40 sm:w-40">
                        Configure a UPI ID in Admin Settings to generate the payment QR code.
                      </div>
                    )}
                  ...
                  <div className="mt-2">
                    ...
                    <Input
                      className="mt-1.5 h-10 rounded-xl border-stone-200 bg-white"
                      value={transactionId}
                      onChange={(e) =>
              ...
              <div className="mt-2 border-t border-stone-200 pt-2">
                <Button
                  type="submit"
                  size="lg"
                  className="h-11 w-full rounded-xl bg-[#2f6b45] text-white hover:bg-[#255536]"
                ...
                {/* <p className="mt-2 text-center text-[11px] leading-tight text-stone-400">
                  By placing the order, you confirm that the delivery details are correct.
                </p> */}
              </div>
            </CardContent>
          </Card>
