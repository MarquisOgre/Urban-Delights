...
// Exactly five order-status steps, matching the customer tracking page.
const statusOptions = [
  ['received', 'Received'],
  ['confirmed', 'Confirmed'],
...
const OrdersList: React.FC<OrdersListProps> = ({ onBackToDashboard }) => {
  ...
  const getTotalQuantity = (order: Order) =>
    `${(order.items || []).reduce((sum, item) => sum + quantityToKg(item.quantity_type), 0).toFixed(3)} Kgs`;
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="min-w-[470px]">Items</TableHead>
                    ...
                    return (
                      <TableRow key={order.id}>
                        <TableCell className="align-top whitespace-nowrap font-medium">
                          {formatInvoiceNumber(order.invoice_number, order.order_date)}
                        </TableCell>
                        ...
                        <TableCell className="align-top p-2">
                          <div className="min-w-[450px]">
                            <div className="mb-1 grid grid-cols-[35px_minmax(155px,1fr)_65px_90px_90px] gap-x-2 border-b pb-1 text-xs font-semibold text-muted-foreground">
                            ...
                            {(order.items || []).map((item, index) => {
                              const rate = getRatePerKg(Number(item.amount || 0), item.quantity_type);
                              return (
                                <div key={`${item.recipe_name}-${index}`} className="grid grid-cols-[35px_minmax(155px,1fr)_65px_90px_90px] gap-x-2 py-0.5 text-xs">
                                  <span className="text-center">{index + 1}</span>
                                  <span>{item.recipe_name}</span>
                                  <span>{item.quantity_type}</span>
                                  <span className="text-right">{rate > 0 ? rate.toFixed(2) : '—'}</span>
                                  <span className="text-right">{formatRupee(item.amount)}</span>
                                </div>
                              );
                            })}
                            {(order.items || []).length > 0 && (
                              <div className="mt-1 grid grid-cols-[35px_minmax(155px,1fr)_65px_90px_90px] gap-x-2 border-t pt-1 text-xs font-semibold">
                                <span />
                                <span>Total Quantity</span>
                                <span>{getTotalQuantity(order)}</span>
                                <span />
                                <span />
                              </div>
                            )}
                        ...
                        <TableCell className="align-top whitespace-nowrap">
                          {totals.discountPercent > 0 ? `-${formatRupee(totals.discount)}` : formatRupee(0)}
                          {totals.discountPercent > 0 && (
                            <div className="text-xs text-muted-foreground">{totals.discountPercent}%</div>
                          )}
                        </TableCell>
                        <TableCell className="align-top whitespace-nowrap">
                          {formatRupee(totals.tax)}
                          <div className="text-xs text-muted-foreground">{totals.taxRate}%</div>
                        </TableCell>
                        <TableCell className="align-top whitespace-nowrap font-medium">
                          {formatRupee(totals.total || Number(order.total_amount || 0))}
                        </TableCell>
                        <TableCell className="align-top">
                          <Select value={order.status || 'received'} onValueChange={(value) => handleStatusChange(order.id, value)}>
                        ...
                        <TableCell className="align-top">
                          <Select value={order.payment_status || 'pending'} onValueChange={(value) => handlePaymentChange(order.id, value)}>
                        ...
                        <TableCell className="align-top">
                          <div className="flex gap-1">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8" title="Download Invoice PDF" onClick={() => downloadInvoicePDF(order)}>
                              <Download className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8" title="Edit Order" onClick={() => setEditingOrder(order)}>
                              <Pencil className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" title="Delete Order" onClick={() => handleDelete(order.id)}>
