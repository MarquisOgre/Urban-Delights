...
const Footer: React.FC<FooterProps> = ({ showTopButton = false }) => {
  ...
  return <>
    <footer className="bg-white">
      {settings.showFooterLinks && links.length > 0 && <nav aria-label="Footer navigation" className="container mx-auto flex w-full flex-wrap items-center justify-center gap-x-8 gap-y-3 border-t border-stone-100 px-6 py-8 text-center text-sm sm:gap-x-10 sm:px-8 sm:py-10 sm:text-base">{links.map((link) => <Link key={`${link.path}-${link.label}`} to={link.path} className="shrink-0 text-stone-600 transition-colors duration-200 hover:text-orange-600" onClick={backToTop}>{link.label}</Link>)}</nav>}
